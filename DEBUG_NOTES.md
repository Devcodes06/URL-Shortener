...
| **Step 4: Live Vercel Status** | Live inspection of production deployment `https://url-shortener-gray-two.vercel.app`. | **CONFIRMED ROOT CAUSE** | Live `/healthz` successfully deployed and returns HTTP 200: `{"ok":false,"mongooseReadyState":0,"hasMongoUri":true,"hasJwtSecret":false,"hasGeminiKey":true,"node":"v24.19.0"}`. Live `POST /user/login` now rejects within 8s (down from 23s hang) with Express HTTP 500 without crashing lambda (`FUNCTION_INVOCATION_FAILED` is eliminated). Crucially, `/healthz` reveals `hasJwtSecret: false` and `mongooseReadyState: 0`, confirming Atlas network/credential blockage from Vercel. |
